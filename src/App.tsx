  ...
  <div className="container mx-auto px-4 py-4">
    <div className="flex flex-col md:flex-row items-start gap-8">
      ...
      <div className="flex-1 w-full md:w-auto">
        <div className="relative w-full max-w-2xl mx-auto">
          ...
          <button
            type="submit"
            aria-label="Search"
            className="absolute right-2 top-1/2 -translate-y-1/2 text-[#00BFFF] hover:text-[#FFD700] transition-colors"
          >
            <Search size={20} />
          </button>
        </div>
      ...
      <div className="flex items-center gap-4 mt-4 md:mt-0 w-full md:w-auto justify-center md:justify-end">
        ...
        <a
          href="https://www.youtube.com/@SPITMedia-tu5rk"
          aria-label="YouTube"
          className="text-white/80 hover:text-[#00BFFF] transition-colors"
          target="_blank"
          rel="noopener noreferrer"
    ...
    {/* Accreditation links */}
    <nav className="mt-4 flex flex-wrap items-center justify-center md:justify-end gap-4 text-sm" aria-label="Accreditation links">
      {[
        { to: '/accreditation/NAAC', label: 'NAAC' },
        { to: '/accreditation/NIRF', label: 'NIRF' },
        { to: '/accreditation/IIC', label: 'IIC' },
        { to: '/accreditation/ARIIA', label: 'ARIIA' },
        ...
        <React.Fragment key={to}>
          <NavLink
            ...
            className={({ isActive }) =>
              isActive
                ? 'text-[#FFD700] font-semibold transition-colors'
                : 'text-white hover:text-[#FFD700] transition-colors'
            }
          >
            {label}
          </NavLink>
          {idx < 4 && <span className="text-[#8B3A3A]">|</span>}
        </React.Fragment>
      ))}
    </nav>
  </div>
