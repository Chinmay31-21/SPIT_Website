...
// Define the different stages for each animation type
type AnimationStageOriginal = 'initial' | 'journey' | 'landing' | 'logo' | 'loading' | 'complete';
type AnimationStageShort = 'initial' | 'loading' | 'complete';


// --- UTILITY: Colorful Fluid Animation Component ---
...
// --- 1. The ORIGINAL, Long Animation Component ---
const PowerUpAnimationOriginal = ({ onComplete }: { onComplete: () => void }) => {
  const [stage, setStage] = useState<AnimationStageOriginal>('initial');
  const [loading, setLoading] = useState(0);
  const [isButtonClicked, setIsButtonClicked] = useState(false);
  const [showSpark, setShowSpark] = useState(false);

  useEffect(() => {
    const stageTimings: { [key in AnimationStageOriginal]?: number } = {
      journey: 3000,
      landing: 2000,
      logo: 1500,
      loading: 3000
    ...
    if (isButtonClicked && stage !== 'complete') {
      const nextStage: { [key in AnimationStageOriginal]: AnimationStageOriginal } = {
        initial: 'journey',
        journey: 'landing',
        landing: 'logo',
        logo: 'loading',
...
// --- 2. The NEW, Short Animation Component ---
const PowerUpAnimationShort = ({ onComplete }: { onComplete: () => void }) => {
  const [stage, setStage] = useState<AnimationStageShort>('initial');
  const [loading, setLoading] = useState(0);
...
// --- 3. The Main CONTROLLER Component ---
...
export const PowerUpAnimation = ({ onComplete }: { onComplete: () => void }) => {
  const [animationType, setAnimationType] = useState<'original' | 'short' | null>(null);

  useEffect(() => {
    const visitCountStr = localStorage.getItem('spitNetworkVisitCount');
    let currentVisitCount = visitCountStr ? parseInt(visitCountStr, 10) : 0;

    // --- CORRECTED LOGIC ---
    // The condition now correctly checks for the 1st, 6th, 12th, etc., visit.
    if (currentVisitCount === 0 || (currentVisitCount > 0 && (currentVisitCount + 1) % 6 === 0)) {
        setAnimationType('original');
    } else {
        setAnimationType('short');
    }

    // Increment and save the count for the next visit.
    localStorage.setItem('spitNetworkVisitCount', (currentVisitCount + 1).toString());
  }, []); // The empty `[]` ensures this runs only once on mount.

  // Render the chosen animation component based on the logic
  if (animationType === 'original') {
    return <PowerUpAnimationOriginal onComplete={onComplete} />;
  }

  if (animationType === 'short') {
    return <PowerUpAnimationShort onComplete={onComplete} />;
  }

  // Render nothing temporarily to prevent any visual flicker
  return null;
};