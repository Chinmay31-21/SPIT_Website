...
import { Download } from 'lucide-react'; // Assuming Lucide React is installed and working
...
const AnnualReport = () => {
  ...
  useEffect(() => {
    // A simple way to trigger stats animation once on mount or when section is visible
    const timer = setTimeout(() => {
      setStatsAnimated(true);
    }, 500); // Delay for initial animation
  ...
  const AnimatedNumber = ({ value, suffix = '' }) => {
    // currentNumber will always store a numerical value
    const [currentNumber, setCurrentNumber] = useState(0); 

    useEffect(() => {
      if (statsAnimated) {
        let start = 0;
        const end = parseFloat(value); // Ensure 'end' is a number
        ...
        const animateCount = (currentTime) => {
          ...
          setCurrentNumber(current); // Update state with the raw numerical value
    ...
    }, [statsAnimated, value]); // Depend on statsAnimated and value
    ...
    if (suffix === '%') {
      ...
      formattedValue = Math.floor(currentNumber); // For integers with a plus sign
    } else {
      formattedValue = Math.floor(currentNumber); // Default to floor for other numbers
    }
  ...
  return (
    <div 
      ...
      style={{
        // Professional background image - replace with a high-quality, relevant image if desired
        backgroundImage: `url('https://images.pexels.com/photos/3184454/pexels-photo-3184454.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1')`,
        backgroundSize: 'cover',
        backgroundPosition: 'center',
        backgroundAttachment: 'fixed', // Creates a subtle parallax effect
      }}
    >
      {/* Dynamic gradient overlay for readability and thematic depth */}
      <div className="absolute inset-0 bg-gradient-to-br from-black/90 to-gray-950/90 backdrop-blur-sm animate-gradient-subtle-shift"></div>
